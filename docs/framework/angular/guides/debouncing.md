---
title: Angular Debouncing Guide
id: debouncing
---
...
The focused snippets later in this guide use `injectDebouncer` and assume they run inside an Angular injection context.
...
`injectDebouncer` does not provide a `maxWait` option. A continuous stream of calls can keep postponing a trailing execution indefinitely. Use [throttling](./throttling.md) when work must continue at a bounded interval while calls are still arriving.
...
```ts
const debouncer = injectDebouncer(saveDraft, { wait: 1000 })
