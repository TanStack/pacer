---
title: Angular Async Batching Guide
id: async-batching
---
...
export class AnalyticsComponent {
  readonly batcher = injectAsyncBatcher(
    sendEvents,
...
The focused snippets later in this guide use `injectAsyncBatcher` and assume they run inside an Angular injection context.
...
If the second batch's trigger fires before the first finishes, both batch functions can overlap. `injectAsyncBatcher` does not have a concurrency option. Serialize batch executions outside the batcher or send the completed batches through an async queue when overlap is unsafe.
...
```ts
const batcher = injectAsyncBatcher(sendEvents, {
  maxSize: 20,
  wait: 1000,
  asyncRetryerOptions: {
    maxAttempts: 3,
