---
title: Angular Async Debouncing Guide
id: async-debouncing
---
...
The focused snippets later in this guide use `injectAsyncDebouncer` and assume they run inside an Angular injection context.
...
```ts
const save = injectAsyncDebouncer(saveDraft, {
  wait: 500,
  asyncRetryerOptions: {
    maxAttempts: 3,
    backoff: 'exponential',
...
```ts
const search = injectAsyncDebouncer(
  async (query: string) => {
