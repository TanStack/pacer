...
- `createDebouncedSignal` or `createDebouncedValue` for delayed reactive values
- `createDebouncer` for callbacks, lifecycle methods, and selected state

Use the signal or value API for delayed reactive state. Use `createDebouncer` for event handlers, lifecycle methods, selected state, or dynamic options.

## Solid example

```tsx
import { createDebouncedSignal, createDebouncer } from '@tanstack/solid-pacer'

function SearchBox() {
  const [query, setQuery] = createDebouncedSignal('', { wait: 300 })
  const saver = createDebouncer(saveDraft, { wait: 500 }, (state) => ({
...
The focused snippets later in this guide use `createDebouncer` and assume they run inside a Solid reactive owner.
...
```ts
const debouncer = createDebouncer(saveDraft, {
  wait: 1000,
  leading: true,
  trailing: true,
})
...
```ts
const debouncer = createDebouncer(saveDraft, { wait: 1000 })
