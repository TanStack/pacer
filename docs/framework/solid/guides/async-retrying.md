---
title: Solid Async Retrying Guide
id: async-retrying
---
...
Retrying is the exception among these framework guides: TanStack Pacer does not provide a Solid-specific retry primitive. The adapter re-exports the public `asyncRetry` function and `AsyncRetryer` class, so this guide uses those APIs and connects long-lived instances to the Solid lifecycle.
...
```ts
import { asyncRetry } from '@tanstack/solid-pacer'
...
```ts
import { AsyncRetryer } from '@tanstack/solid-pacer'
...
onCleanup(() => retryer.abort())

void retryer.execute(props.id)
```

Starting a second `execute()` on the same instance aborts its earlier retry flow. Create separate instances when executions may overlap.
