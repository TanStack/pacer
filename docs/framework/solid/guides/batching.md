...
- `createBatcher` for item addition, lifecycle methods, and selected state

`createBatcher` handles both item addition and instance control. Use `addItem()` for collection, and use the same instance for `flush()`, `cancel()`, selected state, and dynamic options.
...
batcher.addItem({ type: 'click' })
console.log(batcher.state().size)
```

The focused snippets later in this guide use `createBatcher` and assume they run inside a Solid reactive owner.
...
```ts
const batcher = createBatcher<number>(processBatch, {
