  ...
  constructor(
    ...
    if (this.key) {
      pacerEventClient.on('d-AsyncRateLimiter', (event) => {
        ...
        this.#setState(event.payload.store.state as Partial<AsyncRateLimiterState<TFn>>)
        this.setOptions(event.payload.options as Partial<AsyncRateLimiterOptions<TFn>>)
      })
    }
  }
