...
const updateOrAddToArray = <T extends { uuid: string }>(oldArray: Array<T>, newItem: T) => {
    ...
    const cleanup = pacerEventClient.onAllPluginEvents((e) => {
      ...
      switch (e.type) {
        case 'pacer:async-batcher-state':
          {
            const newItems = updateOrAddToArray(store.asyncBatcherState, e.payload)
            setStore({ asyncBatcherState: newItems })
            break
          }
        case 'pacer:async-debouncer-state':
          {
            const newItems = updateOrAddToArray(store.asyncDebouncerState, e.payload)
        ...
        case 'pacer:async-rate-limiter-state':
          {
            const newItems = updateOrAddToArray(store.asyncRateLimiterState, e.payload)
            setStore({ asyncRateLimiterState: newItems })
            break
          }
        case 'pacer:async-throttler-state':
          {
            const newItems = updateOrAddToArray(store.asyncThrottlerState, e.payload)
